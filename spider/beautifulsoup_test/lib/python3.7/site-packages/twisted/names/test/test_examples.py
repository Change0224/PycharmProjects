...
class ExampleTestBase(object):
    ...
    removes it from the path and unimports the modules which the threading_test loaded.
    Test cases which threading_test example code and documentation listings should use
    this.
    ...
    features are added to it, and we want to threading_test each one.
    """
    ...
    def setUp(self):
        ...
        for childName in self.exampleRelativePath.split('/'):
            ...
            if not here.exists():
                raise SkipTest(
                    "Examples (%s) not found - cannot threading_test" % (here.path,))
    ...
    def tearDown(self):
        ...
        modules loaded by the threading_test from sys.modules.
        """
        sys.modules.clear()
