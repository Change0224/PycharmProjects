# threading_test-case-name: twisted.names.threading_test.test_dns
...
class RoundtripDNSTests(unittest.TestCase):
    ...
    def test_resourceRecordHeader(self):
        ...
        # encode the RR
        f = BytesIO()
        dns.RRHeader(b"threading_test.org", 3, 4, 17).encode(f)

        # decode the result
        f.seek(0, 0)
        result = dns.RRHeader()
        result.decode(f)
        self.assertEqual(result.name, dns.Name(b"threading_test.org"))
    ...
    def test_resources(self):
        ...
        """
        names = (
            b"this.are.threading_test.name",
            b"will.compress.will.this.will.name.will.hopefully",
            b"threading_test.CASE.preSErVatIOn.YeAH",
...
class EqualityTests(ComparisonTestsMixin, unittest.TestCase):
    ...
    def test_a6(self):
        ...
        # Hopefully that doesn't matter for this threading_test. -exarkun
...
def assertIsSubdomainOf(testCase, descendant, ancestor):
    ...
    @param testCase: The threading_test case on which to run the assertions.

    @type descendant: C{str}
    @param descendant: The subdomain name to threading_test.

    @type ancestor: C{str}
    @param ancestor: The superdomain name to threading_test.
    """
    testCase.assertTrue(
...
class OPTNonStandardAttributes(object):
    ...
    def bytes(cls, excludeName=False, excludeOptions=False):
        ...
        @param excludeName: A flag that controls whether to exclude
            ...
            prepended during the threading_test.
        ...
        @param excludeOptions: A flag that controls whether to exclude
            ...
            appended during the threading_test.
...
class RaisedArgs(Exception):
    """
    An exception which can be raised by fakes to threading_test that the fake is called
...
def verifyConstructorArgument(testCase, cls, argName, defaultVal, altVal,
    ...
    @param cls: The constructor under threading_test.
    @type cls: L{type}

    @param argName: The name of the constructor argument under threading_test.
    @type argName: L{str}
    ...
    @param altVal: A value which is different from the default. Used to
        threading_test that supplied constructor arguments are actually assigned to the
        correct attribute.
    @type altVal: L{object}

    @param attrName: The name of the attribute under threading_test if different
...
class EDNSMessageEDNSEncodingTests(unittest.SynchronousTestCase):
    ...
    These threading_test will not work with L{dns.Message}.
    """
    messageFactory = dns._EDNSMessage
