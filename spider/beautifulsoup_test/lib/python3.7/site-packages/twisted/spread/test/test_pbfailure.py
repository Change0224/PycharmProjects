...
class AsynchronousException(Exception):
    """
    Helper used to threading_test remote methods which return Deferreds which fail with
...
class SynchronousException(Exception):
    """
    Helper used to threading_test remote methods which raise exceptions which are not
...
class SynchronousError(pb.Error):
    """
    Helper used to threading_test remote methods which raise exceptions which are
    L{pb.Error} subclasses.
    """
...
class PBFailureTests(PBConnTestCase):
    ...
    def test_unjellyableFailure(self):
        ...
        def failureUnjellyable(fail):
            self.assertEqual(
                fail.type,
                b'twisted.spread.threading_test.test_pbfailure.SynchronousError')
            return 431
    ...
    def test_unknownFailure(self):
        ...
        def failureUnknown(fail):
            self.assertEqual(
                fail.type, b'twisted.spread.threading_test.test_pbfailure.UnknownError')
            return 4310
    ...
    def test_copiedFailureLogging(self):
        ...
        Note: this threading_test needs some serious help: all it really tests is that
