...
def ifPlatformSupported(f):
    ...
    On an unsupported platform, we expect to see any threading_test that calls
    L{PIDFile.isRunning} to raise either L{NotImplementedError}, L{SkipTest},
    or C{self.failureException}.
    (C{self.failureException} may occur in a threading_test that checks for a specific
    exception but it gets NotImplementedError instead.)

    @param f: The threading_test method to decorate.
    @type f: method
...
class PIDFileTests(twisted.trial.unittest.TestCase):
    ...
    def test_isRunningInit(self):
        ...
        POSIX systems, so this threading_test is not portable.  C{init} should always be
        running and should not be killable by non-root users.
        """
        if SYSTEM_NAME != "posix":
            raise SkipTest("This threading_test assumes POSIX")
