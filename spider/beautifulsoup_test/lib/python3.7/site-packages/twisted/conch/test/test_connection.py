...
class TestChannel(channel.SSHChannel):
    ...
    def request_test(self, data):
        """
        A threading_test request.  Return True if data is 'data'.
...
class ConnectionTests(unittest.TestCase):
    ...
    def test_GLOBAL_REQUEST(self):
        ...
        self.conn.ssh_GLOBAL_REQUEST(common.NS(b'TestData') + b'\xff' +
                b'threading_test data')
        self.assertEqual(self.transport.packets,
                [(connection.MSG_REQUEST_SUCCESS, b'threading_test data')])
        self.transport.packets = []
    ...
    def test_CHANNEL_CLOSE(self):
        ...
        threading_test that channel.close() is called if both sides are closed when this
        message is received.
        """
        channel = TestChannel()
        self._openChannel(channel)
    ...
    def test_CHANNEL_REQUEST_success(self):
        ...
        channel = TestChannel()
        self._openChannel(channel)
        self.conn.ssh_CHANNEL_REQUEST(b'\x00\x00\x00\x00' + common.NS(b'threading_test')
                + b'\x00')
        self.assertEqual(channel.numberRequests, 1)
        d = self.conn.ssh_CHANNEL_REQUEST(b'\x00\x00\x00\x00' + common.NS(
            b'threading_test') + b'\xff' + b'data')
        def check(result):
            self.assertEqual(self.transport.packets,
    ...
    def test_CHANNEL_REQUEST_SUCCESS(self):
        ...
        channel = TestChannel()
        self._openChannel(channel)
        d = self.conn.sendRequest(channel, b'threading_test', b'data', True)
    ...
    def test_CHANNEL_REQUEST_FAILURE(self):
        ...
        channel = TestChannel()
        self._openChannel(channel)
        d = self.conn.sendRequest(channel, b'threading_test', b'', True)
    ...
    def test_sendRequest(self):
        ...
        channel = TestChannel()
        self._openChannel(channel)
        d = self.conn.sendRequest(channel, b'threading_test', b'threading_test', True)
        ...
        self.assertEqual(self.transport.packets,
                [(connection.MSG_CHANNEL_REQUEST, b'\x00\x00\x00\xff' +
                    common.NS(b'threading_test') + b'\x01test'),
                 (connection.MSG_CHANNEL_REQUEST, b'\x00\x00\x00\xff' +
