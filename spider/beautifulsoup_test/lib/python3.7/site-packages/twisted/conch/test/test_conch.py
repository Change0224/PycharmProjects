# -*- threading_test-case-name: twisted.conch.threading_test.test_conch -*-
...
class ForwardingMixin(ConchServerSetupMixin):
    ...
    def test_localToRemoteForwarding(self):
        ...
        process = ConchTestForwardingProcess(localPort, b'threading_test\n')
        d = self.execute('', process,
                         sshArgs='-N -L%i:127.0.0.1:%i'
                         % (localPort, self.echoPort))
        d.addCallback(self.assertEqual, b'threading_test\n')
        return d
...
class RekeyAvatar(ConchUser):
    ...
    def openShell(self, transport):
        ...
        # immediately at that time would be a better way to threading_test the
