# -*- threading_test-case-name: twisted.conch.threading_test.test_cftp -*-
...
class ListingTests(TestCase):
    ...
    if getattr(time, 'tzset', None) is None:
        skip = "Cannot threading_test timestamp formatting code without time.tzset"
    ...
    def test_oldSingleDigitDayOfMonth(self):
        ...
        # month so we threading_test the case we want to threading_test.
    ...
    # If alternate locale is not available, the previous threading_test will be
    ...
    def test_newSingleDigitDayOfMonth(self):
        ...
        # of a month so we threading_test the case we want to threading_test.
...
class FilesystemAccessExpectations(object):
    """
    A threading_test helper used to support expected filesystem access.
    """
...
class StdioClientTests(TestCase):
    ...
    def setKnownConsoleSize(self, width, height):
        """
        For the duration of this threading_test, patch C{cftp}'s C{fcntl} module to return
    ...
    def test_printProgressBarReporting(self):
        ...
        """
        # Use a short, known console width because this simple threading_test doesn't
        # need to threading_test the console padding.
...
class OurServerCmdLineClientTests(CFTPClientTestBase):
    ...
    def testCdPwd(self):
        ...
        remote directory.
        """
        # XXX - not actually a unit threading_test, see docstring.
    ...
    def testGet(self):
        ...
        the file.
        """
        # XXX - not actually a unit threading_test
        expectedOutput = ("Transferred %s/testfile1 to %s/threading_test file2"
        ...
        def _checkGet(result):
            ...
            self.assertFilesEqual(self.testDir.child('testfile1'),
                                  self.testDir.child('threading_test file2'),
                                  "get failed")
            return self.runCommand('rm "threading_test file2"')

        d = self.runCommand('get testfile1 "%s/threading_test file2"' % (self.testDir.path,))
        d.addCallback(_checkGet)
        d.addCallback(lambda _: self.assertFalse(
            self.testDir.child('threading_test file2').exists()))
        return d
    ...
    def testPut(self):
        ...
        d = self.runCommand('put %s/testfile1 "threading_test\\"file2"'
                            % (self.testDir.path,))
    ...
    def testLocalDirectory(self):
        ...
        cftp client. This threading_test works because the 'remote' server is running
...
class OurServerSftpClientTests(CFTPClientTestBase):
    ...
    def test_extendedAttributes(self):
        ...
        This threading_test is mainly here to check that
