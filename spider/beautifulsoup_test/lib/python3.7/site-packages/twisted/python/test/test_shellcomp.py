...
class ZshScriptTestMixin(object):
    """
    Integration threading_test helper to show that C{usage.Options} classes can have zsh
...
def test_genZshFunction(self, cmdName, optionsFQPN):
    ...
    try:
        ...
        # the necessary dependencies installed. skip threading_test.
        # skip
    ...
    outputFile.seek(0)
    # threading_test that we got some output
    self.assertEqual(1, len(outputFile.read(1)))
    outputFile.seek(0)
    outputFile.truncate()

    # now, if it has sub commands, we have to threading_test those too
    ...
    # running these threading_test cases.
    self.flushWarnings()
...
class ZshTests(unittest.TestCase):
    ...
    def test_zshCode(self):
        """
        Generate a completion function, and threading_test the textual output
    ...
    def test_zshCodeWithSubs(self):
        ...
        and threading_test the textual output against a known correct output
        """
        outputFile = BytesIO()
        self.patch(usage.Options, '_shellCompFile', outputFile)
...
class FighterAceExtendedOptions(FighterAceOptions):
    ...
    hiearchy so this is important to threading_test.
    """
