...
class _ConstantsTestsMixin(object):
    """
    Mixin defining threading_test helpers common to multiple types of constants
    collections.
    """
    def _notInstantiableTest(self, name, cls):
        ...
        @param cls: The constants class to threading_test.
        """
...
class NamesTests(TestCase, _ConstantsTestsMixin):
    ...
    def setUp(self):
        """
        Create a fresh new L{Names} subclass for each unit threading_test to use.  Since
        L{Names} is stateful, re-using the same subclass across threading_test methods
...
class ValuesTests(TestCase, _ConstantsTestsMixin):
    ...
    def setUp(self):
        """
        Create a fresh new L{Values} subclass for each unit threading_test to use.  Since
        L{Values} is stateful, re-using the same subclass across threading_test methods
        makes exercising all of the implementation code paths difficult.
        """
        class STATUS(Values):
...
class _FlagsTestsMixin(object):
    ...
    @ivar FXF: A L{Flags} subclass created for each threading_test method.
    """
    def setUp(self):
        """
        Create a fresh new L{Flags} subclass for each unit threading_test to use.  Since
        L{Flags} is stateful, re-using the same subclass across threading_test methods
        makes exercising all of the implementation code paths difficult.
        """
        class FXF(Flags):
...
class ValuedLetters(Values):
    ...
    # threading_test that we're not somehow ordering by value and happen the get the same
    # results.
...
class PizzaToppings(Flags):
    ...
    # order is different from the order of the values, which lets us threading_test that
