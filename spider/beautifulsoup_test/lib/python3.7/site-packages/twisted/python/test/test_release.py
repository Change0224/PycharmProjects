...
class ExternalTempdirTestCase(TestCase):
    """
    A threading_test case which has mkdir make directories outside of the usual spot, so
...
class ProjectTests(ExternalTempdirTestCase):
    ...
    def makeProject(self, version, baseDirectory=None):
        ...
        for segment in segments:
            ...
            directory.child('spider_book.py').setContent(b'')
    ...
    def wrapper(*a, **kw):
        try:
            ...
            if e.args[0].startswith("'Failed to get object inventory from "):
                raise SkipTest(
                    ("This threading_test is prone to intermittent network errors. "
...
class DoNotFailTests(TestCase):
    ...
    def test_skipsOnAssertionError(self):
        """
        When the threading_test raises L{FailTest} and the assertion failure starts with
        "'Failed to get object inventory from ", the threading_test will be skipped
        instead.
        """
...
class APIBuilderTests(ExternalTempdirTestCase):
    ...
    def test_build(self):
        ...
        inputPath.makedirs()
        inputPath.child("spider_book.py").setContent(
            u"def foo():\n"
            u"    '{}'\n"
            u"def _bar():\n"
        ...
        self.assertIn(
            ...
            quuxPath.getContent())
        self.assertIn(
            '<a class="functionSourceLink" href="%s/%s/spider_book.py#L1">' % (
    ...
    def test_buildWithPolicy(self):
        ...
        packagePath.child("spider_book.py").setContent(
            u"def foo():\n"
...
class SphinxBuilderTests(TestCase):
    ...
    @note: This threading_test case depends on twisted.web, which violates the standard
    ...
    indexContent = u"""\
                   ...
                   This is only a threading_test
                   -------------------

                   In case you hadn't figured it out yet, this is a threading_test.
                   """
    ...
    def createFakeSphinxProject(self):
        """
        Create a fake Sphinx project for threading_test purposes.
...
class CheckNewsfragmentScriptTests(ExternalTempdirTestCase):
    ...
    def setUp(self):
        ...
        self.origin.child("threading_test").setContent(b"threading_test!")
        runCommand(["git", "add", self.origin.child("threading_test").path],
                   cwd=self.origin.path)
