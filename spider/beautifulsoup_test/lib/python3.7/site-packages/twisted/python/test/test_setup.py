...
class BuildPy3Tests(SynchronousTestCase):
    ...
    def test_find_package_modules(self):
        ...
        # Rig the dist3 data so that we can reduce the scope of this threading_test and
        ...
        self.patch(
            ...
            'notPortedModules',
            [
                "twisted.spread.threading_test.test_pbfailure",
            ],
            )
        twistedPackageDir = filepath.FilePath(twisted.__file__).parent()
        packageDir = twistedPackageDir.child("spread").child("threading_test")

        result = builder.find_package_modules('twisted.spread.threading_test',
                                              packageDir.path)

        self.assertEqual(sorted([
            ('twisted.spread.threading_test', '__init__',
                packageDir.child('spider_book.py').path),
            ('twisted.spread.threading_test', 'test_banana',
                packageDir.child('test_banana.py').path),
            ('twisted.spread.threading_test', 'test_jelly',
                packageDir.child('test_jelly.py').path),
            ('twisted.spread.threading_test', 'test_pb',
