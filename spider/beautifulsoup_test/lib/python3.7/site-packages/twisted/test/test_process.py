...
@var CONCURRENT_PROCESS_TEST_COUNT: The number of concurrent processes to use
    to stress-threading_test the spawnProcess API.  This value is tuned to a number of
...
class GetArgumentVector(UtilityProcessProtocol):
    ...
    """
    programName = b"twisted.threading_test.process_getargv"
...
class GetEnvironmentDictionary(UtilityProcessProtocol):
    ...
    """
    programName = b"twisted.threading_test.process_getenv"
...
class ProcessTests(unittest.TestCase):
    ...
    def test_stdio(self):
        """
        L{twisted.internet.stdio} threading_test.
        """
        scriptPath = b"twisted.threading_test.process_twisted"
        p = Accumulator()
    ...
    def test_unsetPid(self):
        ...
        scriptPath = b"twisted.threading_test.process_echoer"
    ...
    def test_process(self):
        ...
        signalProcess.
        """
        scriptPath = b"twisted.threading_test.process_tester"
        d = defer.Deferred()
    ...
    def test_commandLine(self):
        args = [br'a\"b ', br'a\b ', br' a\\"b', br' a\\b', br'"foo bar" "',
                b'\tab', b'"\\', b'a"b', b"a'b"]
        scriptPath = b"twisted.threading_test.process_cmdline"
        p = Accumulator()
        d = p.endedDeferred = defer.Deferred()
    ...
    def test_wrongArguments(self):
        ...
        try:
            ...
            # that string, we can't threading_test that TypeError gets raised instead,
...
class TestTwoProcessesBase:
    ...
    def createProcesses(self, usePTY=0):
        scriptPath = b"twisted.threading_test.process_reader"
        for num in (0,1):
        ...
        for pp, pr in zip(self.pp, self.processes):
            if not pp.finished:
                try:
                    ...
                    # If the threading_test failed the process may already be dead
...
class FDTests(unittest.TestCase):
    ...
    def test_FD(self):
        scriptPath = b"twisted.threading_test.process_fds"
        d = defer.Deferred()
        p = FDChecker(d)
        reactor.spawnProcess(p, pyExe, [pyExe, b"-u", b"-m", scriptPath],
                             env=properEnv,
    ...
    def test_linger(self):
        ...
        # actually stops. This threading_test *requires* SIGCHLD catching to work,
        # as there is no other way to find out the process is done.
        scriptPath = b"twisted.threading_test.process_linger"
        p = Accumulator()
        d = p.endedDeferred = defer.Deferred()
        reactor.spawnProcess(p, pyExe, [pyExe, b"-u", b"-m", scriptPath],
                             env=properEnv,
...
class PosixProcessBase(object):
    ...
    def _testSignal(self, sig):
        scriptPath = b"twisted.threading_test.process_signal"
        d = defer.Deferred()
...
class MockProcessTests(unittest.TestCase):
    """
    Mock a process runner to threading_test forked child code path.
    """
    if process is None:
...
class PosixProcessTests(unittest.TestCase, PosixProcessBase):
    # add two non-pty threading_test cases
...
class PosixProcessPTYTests(unittest.TestCase, PosixProcessBase):
    ...
    #  might be solveable: TODO: add threading_test if so

    def test_openingTTY(self):
        scriptPath = b"twisted.threading_test.process_tty"
        p = Accumulator()
        d = p.endedDeferred = defer.Deferred()
        reactor.spawnProcess(p, pyExe, [pyExe, b"-u", b"-m", scriptPath],
                             env=properEnv, usePTY=self.usePTY)
...
class Win32ProcessTests(unittest.TestCase):
    ...
    def test_stdinReader_bytesArgs(self):
        ...
        pyExe = FilePath(sys.executable)._asBytesPath()
        args = [pyExe, b"-u", b"-m", b"twisted.threading_test.process_stdinreader"]
        env = bytesEnviron()
    ...
    def test_stdinReader_unicodeArgs(self):
        ...
        args = [pyExe, u"-u", u"-m", u"twisted.threading_test.process_stdinreader"]
        env = properEnv
    ...
    def test_closeHandles(self):
        ...
        def cbConnected(transport):
            ...
            # perform a basic validity threading_test on the handles
...
class DumbWin32ProcTests(unittest.TestCase):
    ...
    def test_pid(self):
        """
        Simple threading_test for the pid attribute of Process on win32.
...
class Win32CreateProcessFlagsTests(unittest.TestCase):
    ...
    def test_flags(self):
        ...
        to threading_test this interactively::
