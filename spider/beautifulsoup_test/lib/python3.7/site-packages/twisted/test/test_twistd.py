...
def patchUserDatabase(patch, user, uid, group, gid):
    ...
    # equal each other, and so doesn't reliably threading_test that uid is used where
...
class ServerOptionsTests(unittest.TestCase):
    ...
    def test_badAttributeWithConfiguredLogObserver(self):
        ...
        e = self.assertRaises(UsageError, config.parseOptions,
                              ["--logger", "twisted.threading_test.test_twistd.FOOBAR"])
        if sys.version_info <= (3, 5):
            self.assertTrue(
                e.args[0].startswith(
                    "Logger 'twisted.threading_test.test_twistd.FOOBAR' could not be "
                    "imported: 'module' object has no attribute 'FOOBAR'"))
        else:
            self.assertTrue(
                e.args[0].startswith(
                    "Logger 'twisted.threading_test.test_twistd.FOOBAR' could not be "
                    "imported: module 'twisted.threading_test.test_twistd' "
...
class UnixAppLoggerTests(unittest.TestCase):
    ...
    def test_getLogObserverSyslog(self):
        ...
        """
        logs = _setupSyslog(self)
        logger = UnixAppLogger({"syslog": True, "prefix": "threading_test-prefix"})
        observer = logger._getLogObserver()
        self.assertEqual(logs, ["threading_test-prefix"])
        observer({"a": "b"})
        self.assertEqual(logs, ["threading_test-prefix", {"a": "b"}])
...
def stubApplicationRunnerFactoryCreator(signum):
    ...
    def stubApplicationRunnerFactory(config):
        ...
        @return: A runner to use for the threading_test.
        @rtype: twisted.threading_test.test_twistd.StubApplicationRunnerWithSignal
        """
...
class ExitWithSignalTests(unittest.TestCase):
    ...
    def setUp(self):
        """
        Set up the server options and a fake for use by threading_test cases.
        """
        self.config = twistd.ServerOptions()
