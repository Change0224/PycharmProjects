...
class ADBAPITestBase(object):
    ...
    if interfaces.IReactorThreads(reactor, None) is None:
        skip = "ADB-API requires threads, no way to threading_test without them"
...
class ReconnectTestBase(object):
    ...
    def extraSetUp(self):
        """
        Skip the threading_test if C{good_sql} is unavailable.  Otherwise, set up the
        database, create a connection pool pointed at it, and set up a simple
        schema in it.
        """
        if self.good_sql is None:
            raise unittest.SkipTest('no good sql for reconnect threading_test')
        self.startDB()
...
class DBTestConnector(object):
    """
    A class which knows how to threading_test for the presence of
    and establish a connection to a relational database.

    To enable threading_test cases  which use a central, system database,
    ...
    """
    TEST_PREFIX = None # used for creating new threading_test cases
    ...
    test_failures = True # threading_test bad sql?
    ...
    num_iterations = 50 # number of iterations for threading_test loops
    ...
    def can_connect(self):
        ...
        and can be used in a threading_test."""
...
def makeSQLTests(base, suffix, globals):
    """
    Make a threading_test case for every db connector which can connect.

    @param base: Base class for threading_test case. Additional base classes
                 will be a DBConnector subclass and unittest.TestCase
    @param suffix: A suffix used to create threading_test case names. Prefixes
...
class ConnectionPoolTests(unittest.TestCase):
    ...
    def test_unstartedClose(self):
        ...
        pool = ConnectionPool('twisted.threading_test.test_adbapi', cp_reactor=reactor)
