...
"""
I{Private} threading_test utilities for use throughout Twisted's threading_test suite.  Unlike
...
@note: Maintainers be aware: things in this module should be gradually promoted
    to more full-featured threading_test helpers and exposed as public API as your
    maintenance time permits.  In order to be public API though, they need
    their own threading_test cases.
"""
...
class XMLAssertionMixin(object):
    ...
    Must be mixed in to a L{threading_test case<unittest.TestCase>}.
    """
...
class ComparisonTestsMixin(object):
    ...
    Use this to unit threading_test objects which follow the common convention for C{==}
    and C{!=}:
