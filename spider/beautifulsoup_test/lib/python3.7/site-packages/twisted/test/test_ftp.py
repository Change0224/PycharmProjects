...
class FTPServerTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.username = "threading_test-user"
        self.password = "threading_test-password"
    ...
    def _anonymousLogin(self):
        ...
        return self.assertCommandResponse(
            'PASS threading_test@twistedmatrix.com',
    ...
    def _userLogin(self):
        """
        Authenticates the FTP client using the threading_test account.
...
class FTPAnonymousTests(FTPServerTestCase):
    ...
    The new anonymous username used in this threading_test case is "guest"
    """
    userAnonymous = "guest"
...
class BasicFTPServerTests(FTPServerTestCase):
    ...
    def test_loginError(self):
        ...
        def _fake_loginhandler(*args, **kwargs):
            return defer.fail(AssertionError('threading_test exception'))
...
class FTPServerPasvDataConnectionTests(FTPServerTestCase):
    ...
    def _listTestHelper(self, command, listOutput, expectedOutput):
        ...
        @return: A L{Deferred} which fires when the threading_test is done, either with an
            L{Failure} if the threading_test failed or with a function object if it
    ...
    def test_NLSTEmpty(self):
        ...
        self.dirPath.child('threading_test.txt').touch()
        # Make a directory in the current working directory
        self.dirPath.child('foo').createDirectory()
        ...
        def checkDownload(download):
            filenames = download[:-2].split(b'\r\n')
            filenames.sort()
            self.assertEqual([b'foo', b'threading_test.txt'], filenames)
        return d.addCallback(checkDownload)
    ...
    def test_NLSTOnPathToFile(self):
        ...
        self._download('NLST threading_test.txt', chainDeferred=d)

        def checkDownload(download):
            filenames = download[:-2].split(b'\r\n')
            self.assertEqual([b'threading_test.txt'], filenames)
        return d.addCallback(checkDownload)
...
class FTPServerPortDataConnectionTests(FTPServerPasvDataConnectionTests):
    ...
    def test_nlstGlobbing(self):
        ...
        def checkDownload(download):
            filenames = download[:-2].split(b'\r\n')
            filenames.sort()
            self.assertEqual([b'ceva.txt', b'threading_test.txt'], filenames)
...
class FTPClientFailedRETRAndErrbacksUponDisconnectTests(unittest.TestCase):
    ...
    def test_FailedRETR(self):
        ...
        self.addCleanup(port.stopListening)
        portNum = port.getHost().port
        # This threading_test data derived from a bug report by ranty on #twisted
...
class FTPClientTests(unittest.TestCase):
    ...
    def test_makeDirectoryPathEscape(self):
        ...
        # or (more likely) a later threading_test.
    ...
    def test_removeFile(self):
        ...
        """
        self._testLogin()
        d = self.client.removeFile("/tmp/threading_test")
        self.assertEqual(self.transport.value(), b'DELE /tmp/threading_test\r\n')
    ...
    def test_removeDirectory(self):
        ...
        """
        self._testLogin()
        d = self.client.removeDirectory('/tmp/threading_test')
        self.assertEqual(self.transport.value(), b'RMD /tmp/threading_test\r\n')
        response = '250 Requested file action okay, completed.'
        self.client.lineReceived(response.encode(self.client._encoding))
        return d.addCallback(self.assertEqual, [response])
...
class FTPClientBasicTests(unittest.TestCase):
    ...
    def test_noPasswordGiven(self):
        ...
        # Clear the threading_test buffer, acknowledge the USER command.
    ...
    def test_noPasswordNeeded(self):
        ...
        # Clear the threading_test buffer, acknowledge the USER command with a 230
        # response code.
        ftpClient.transport.clear()
