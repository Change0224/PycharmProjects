...
class TunnelTests(SynchronousTestCase):
    """
    L{Tunnel} is mostly tested by other threading_test cases but some tests don't fit
...
class TunnelDeviceTestsMixin(object):
    ...
    def setUp(self):
        """
        Create the L{_IInputOutputSystem} provider under threading_test and open a tunnel
        using it.
        """
    ...
    def test_interface(self):
        """
        The object under threading_test provides L{_IInputOutputSystem}.
        """
    ...
    def test_send(self):
        ...
        # To avoid really inconvenient threading_test failures where the threading_test just hangs
        # forever, set up a timeout for blocking socket operations.  This
        # shouldn't ever be triggered when the threading_test is passing.  It only serves
        # to make sure the threading_test runs eventually completes if something is
        ...
        # Start listening for the threading_test datagram first.  The resulting port
...
class FakeDeviceTestsMixin(object):
    """
    Define a mixin for use with threading_test cases that require an
