# -*- threading_test-case-name: twisted.trial.threading_test.test_util -*-
...
@var DEFAULT_TIMEOUT_DURATION: The default timeout which will be applied to
    asynchronous (ie, Deferred-returning) threading_test methods, in seconds.
"""
...
class DirtyReactorAggregateError(Exception):
    ...
    left in an unclean state after a threading_test.
...
class _Janitor(object):
    ...
    @ivar threading_test: The L{TestCase} to report errors about.
    ...
    def __init__(self, test, result, reactor=None):
        """
        @param test: See L{_Janitor.threading_test}.
    ...
    def postCaseCleanup(self):
        """
        Called by L{unittest.TestCase} after a threading_test to catch any logged errors
    ...
    def postClassCleanup(self):
        """
        Called by L{unittest.TestCase} after the last threading_test in a C{TestCase}
    ...
    def _cleanThreads(self):
        ...
        if interfaces.IReactorThreads.providedBy(reactor):
            if reactor.threadpool is not None:
                ...
                # This improves threading_test isolation somewhat (although this is a
...
class _WorkingDirectoryBusy(Exception):
    """
    A working directory was specified to the runner, but another threading_test run is
