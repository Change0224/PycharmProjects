...
"Compatibility" is a difficult things to define. In practice, it means that:
 ...
 - L{twisted.trial.unittest.TestCase} objects should be able to be used by
   other threading_test runners without those runners requiring special support for
   Trial tests.
 ...
 - Tests that subclass the standard library C{TestCase} and don't do anything
   ...
   threading_test runner without the authors of those tests having to jump through
   hoops.

 - Tests that implement the interface provided by the standard library
   C{TestCase} should be runnable by the Trial runner.

 - The Trial threading_test runner and Trial L{unittest.TestCase} objects ought to be
...
Running tests in a temporary directory should be a feature of the threading_test case,
...
breaking) when run in a different threading_test runner. Further, many tests don't even
