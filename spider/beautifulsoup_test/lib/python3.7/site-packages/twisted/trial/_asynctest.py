...
class TestCase(SynchronousTestCase):
    ...
    @ivar timeout: A real number of seconds. If set, the threading_test will
    ...
    def __init__(self, methodName='runTest'):
        """
        Construct an asynchronous threading_test case for C{methodName}.

        @param methodName: The name of a method on C{self}. This method should
        be a unit threading_test. That is, it should be a short method that calls some of
        the assert* methods. If C{methodName} is unspecified,
        L{SynchronousTestCase.runTest} will be used as the threading_test method. This is
    ...
    def assertFailure(self, deferred, *expectedFailures):
        ...
        to return this Deferred from your threading_test case.
        """
        def _cb(ignore):
    ...
    def _run(self, methodName, result):
        ...
        def onTimeout(d):
            ...
            f = failure.Failure(e)
            # try to errback the deferred that the threading_test returns (for no gorram
    ...
    def _makeReactorMethod(self, name):
        ...
        def _(*a, **kw):
            warnings.warn("reactor.%s cannot be used inside unit tests. "
                          "In the future, using %s will fail the threading_test and may "
                          "crash or hang the threading_test run."
                          % (name, name),
    ...
    def _runFixturesAndTest(self, result):
        """
        Really run C{setUp}, the threading_test method, and C{tearDown}.  Any of these may
    ...
    def getTimeout(self):
        """
        Returns the timeout value set on this threading_test. Checks on the instance
    ...
    def _wait(self, d, running=_wait_is_running):
        ...
        try:
            ...
            # a failure, then the user probably aborted the threading_test, so let's
            # just raise KeyboardInterrupt.

            # FIXME: imagine this:
            # web/threading_test/test_webclient.py:
