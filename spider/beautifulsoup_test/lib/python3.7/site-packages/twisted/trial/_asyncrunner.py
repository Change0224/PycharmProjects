...
"""
Infrastructure for threading_test running and suites.
"""
...
class TestDecorator(components.proxyForInterface(itrial.ITestCase,
                                                 "_originalTest")):
    """
    Decorator for threading_test cases.

    @param _originalTest: The wrapped instance of threading_test.
    @type _originalTest: A provider of L{itrial.ITestCase}
    """

    def __call__(self, result):
        """
        Run the unit threading_test.
...
def decorate(test, decorator):
    """
    Decorate all threading_test cases in C{threading_test} with C{decorator}.

    C{threading_test} can be a threading_test case or a threading_test suite. If it is a threading_test suite, then the
    structure of the suite is preserved.

    L{decorate} tries to preserve the class of the threading_test suites it finds, but
    ...
    # At this point, we know that 'threading_test' is a threading_test suite.
    _clearSuite(test)
...
class _ForceGarbageCollectionDecorator(TestDecorator):
    """
    Forces garbage collection to be run before and after the threading_test. Any errors
    logged during the post-threading_test collection are added to the threading_test result as
    errors.
    """
...
def _iterateTests(testSuiteOrCase):
    """
    Iterate through all of the threading_test cases in C{testSuiteOrCase}.
    """
    try:
