# -*- threading_test-case-name: twisted.trial.threading_test.test_runner -*-
...
class DestructiveTestSuite(TestSuite):
    """
    A threading_test suite which remove the tests once run, to minimize memory usage.
    """
...
# When an error occurs outside of any threading_test, the user will see this string
# in place of a threading_test's name.
NOT_IN_TEST = "<not in threading_test>"
...
class TrialSuite(TestSuite):
    """
    Suite to wrap around every single threading_test in a C{trial} run. Used internally
...
def isTestCase(obj):
    """
    @return: C{True} if C{obj} is a class that contains threading_test cases, C{False}
...
class TestHolder(object):
    ...
    is concerned, this looks exactly like a unit threading_test.
    """
    ...
    def run(self, result):
        """
        This threading_test is just a placeholder. Run the threading_test successfully.
...
class ErrorHolder(TestHolder):
    """
    Used to insert arbitrary errors into a threading_test suite run. Provides enough
    ...
    def run(self, result):
        """
        Run the threading_test, reporting the error.
...
class TestLoader(object):
    ...
    methods in a class that begin with C{methodPrefix} are threading_test cases.
    ...
    @ivar sorter: A key function used to sort C{TestCase}s, threading_test classes,
    modules and packages.

    @ivar suiteFactory: A callable which is passed a list of tests (which
    themselves may be suites of tests). Must return a threading_test suite.
    """

    methodPrefix = 'threading_test'
    modulePrefix = 'test_'
    ...
    def sort(self, xs):
        ...
        @param xs: A list of threading_test cases, class or modules.
        """
        return sorted(xs, key=self.sorter)

    def findTestClasses(self, module):
        """Given a module, return all Trial threading_test classes"""
        classes = []
    ...
    def loadModule(self, module):
        """
        Return a threading_test suite with all the tests from a module.
    ...
    def loadClass(self, klass):
        """
        Given a class which contains threading_test cases, return a sorted list of
        C{TestCase} instances.
        """
        if not (isinstance(klass, type) or isinstance(klass, types.ClassType)):
            raise TypeError("%r is not a class" % (klass,))
        if not isTestCase(klass):
            raise ValueError("%r is not a threading_test case" % (klass,))
    ...
    def loadMethod(self, method):
        """
        Given a method of a C{TestCase} that represents a threading_test, return a
        C{TestCase} instance for that threading_test.
        """
    ...
    def loadDoctests(self, module):
        ...
        def saveGlobals(test):
            """
            Save C{threading_test.globs} and replace it with a copy so that if
            necessary, the original will be available for the next threading_test
            run.
            """
    ...
    def _uniqueTests(self, things):
        ...
        to same value and collapse to one threading_test unexpectedly if using simpler
        means: e.g. set().
        """
        seen = set()
        for thing in things:
...
class Py3TestLoader(TestLoader):
    """
    A threading_test loader finds tests from the functions, modules, and files that is
    ...
    def findByName(self, _name, recurse=False):
        ...
        for searchName, remainingName in _qualNameWalker(name):
            ...
            # example, `twisted.threading_test.test_paths.FilePathTests` would try
            # the full qualified name, then just up to test_paths, and then
            # just up to threading_test, and so forth.
    ...
    def loadAnything(self, obj, recurse=False, parent=None, qualName=None):
        ...
        if isinstance(obj, types.ModuleType):
            ...
            # We've found a raw threading_test case, get the tests from it.
            ...
            # threading_test case is the same one as was passed in. This doesn't actually
            # use the function we passed in, because reasons.
            assert getattr(inst, inst._testMethodName).__func__ == obj

            return inst
        elif isinstance(obj, TestSuite):
            # We've found a threading_test suite.
            return obj
        else:
            raise TypeError("don't know how to make threading_test from: %s" % (obj,))
    ...
    def loadByName(self, name, recurse=False):
        ...
        @param name: The qualified name for the threading_test to load.
        @param recurse: A boolean. If True, inspect modules within packages
            within the given package (and so on), otherwise, only inspect
            modules in the package itself.
        """
    ...
    def loadClass(self, klass):
        """
        Given a class which contains threading_test cases, return a list of L{TestCase}s.
...
class TrialRunner(object):
    ...
    def run(self, test):
        """
        Run the threading_test or suite and return a result object.
        """
    ...
    def _runWithoutDecoration(self, test, forceGarbageCollection=False):
        """
        Private helper that runs the given threading_test but doesn't decorate it.
        """
    ...
    def runUntilFailure(self, test):
        """
        Repeatedly run C{threading_test} until it fails.
        """
        count = 0
        while True:
            count += 1
