...
class CapturingReporter(object):
    ...
    def startTest(self, method):
        """
        Report the beginning of a run of a single threading_test method
    ...
    def stopTest(self, method):
        """
        Report the status of a single threading_test method
        @param method: an object that is adaptable to ITestMethod
        """
...
class TrialRunnerTestsMixin(object):
    ...
    def test_empty(self):
        """
        Empty threading_test method, used by the other tests.
        """
...
class DryRunMixin(object):
    ...
    def makeTestFixtures(self):
        """
        Set C{self.threading_test} and C{self.suite}, where C{self.suite} is an empty
        TestSuite.
        """
    ...
    def test_singleCaseReporting(self):
        """
        If we are running a single threading_test, check the reporter starts, passes and
        then stops the threading_test during a dry run.
        """
...
class RunnerTests(unittest.SynchronousTestCase):
    ...
    def test_concurrentImplicitWorkingDirectory(self):
        ...
        # Make sure we end up with the same working directory after this threading_test
        # as we had before it.
        ...
        class ConcurrentCase(unittest.SynchronousTestCase):
            def test_first(self):
                """
                Start a second threading_test run which will have a default working
                directory which is the same as the working directory of the
                threading_test run already in progress.
                """
                # Change the working directory to the value it had before this
                # threading_test suite was started.
    ...
    def test_runner_normal(self):
        self.parseOptions(['--temp-directory', self.mktemp(),
                           '--reporter', 'capturing',
                           'twisted.trial.threading_test.sample'])
        my_runner = self.getRunner()
        loader = runner.TestLoader()
        suite = loader.loadByName('twisted.trial.threading_test.sample', True)
    ...
    def test_runnerDebug(self):
        ...
        self.parseOptions(['--reporter', 'capturing',
                           '--debug', 'twisted.trial.threading_test.sample'])
        my_runner = self.getRunner()
    ...
    def test_runnerDebuggerDefaultsToPdb(self):
        ...
        """
        self.parseOptions(['--debug', 'twisted.trial.threading_test.sample'])
    ...
    def test_runnerDebugger(self):
        ...
        self.parseOptions([
            '--reporter', 'capturing',
            '--debugger',
            'twisted.trial.threading_test.test_runner.RunnerTests.cdebugger',
            '--debug',
            'twisted.trial.threading_test.sample',
        ])
        my_runner = self.getRunner()
    ...
    def test_exitfirst(self):
        """
        If trial was passed the C{--exitfirst} option, the constructed threading_test
...
class TrialSuiteTests(unittest.SynchronousTestCase):
    ...
    def test_imports(self):
        # FIXME, HTF do you threading_test the reactor can be cleaned up ?!!!
...
class UntilFailureTests(unittest.SynchronousTestCase):
    class FailAfter(pyunit.TestCase):
        """
        A threading_test case that fails when run 3 times in a row.
        """
        count = []
        def test_foo(self):
...
class BreakingSuite(runner.TestSuite):
    ...
    def run(self, result):
        try:
            raise RuntimeError("error that occurs outside of a threading_test")
        except RuntimeError:
...
class LoggedErrorsTests(unittest.SynchronousTestCase):
    """
    It is possible for an error generated by a threading_test to be logged I{outside} of
    any threading_test. The log observers constructed by L{TestCase} won't catch these
...
class ErrorHolderTestsMixin(object):
    """
    This mixin defines threading_test methods which can be applied to a
...
class MalformedMethodTests(unittest.SynchronousTestCase):
    """
    Test that trial manages when threading_test methods don't have correct signatures.
    """
    class ContainMalformed(pyunit.TestCase):
        """
        This TestCase holds malformed threading_test methods that trial should handle.
        """
    ...
    def _test(self, method):
        """
        Wrapper for one of the threading_test method of L{ContainMalformed}.
        """
...
class DestructiveTestSuiteTests(unittest.SynchronousTestCase):
    ...
    def test_basic(self):
        """
        Thes destructive threading_test suite should run the tests normally.
        """
        called = []
    ...
    def test_shouldStop(self):
        ...
        # The last threading_test shouldn't have been run
        self.assertEqual(suite.countTestCases(), 1)


    def test_cleanup(self):
        """
        Checks that the threading_test suite cleanups its tests during the run, so that
        it ends empty.
        """
        class MockTest(pyunit.TestCase):
            def test_foo(test):
...
class RunnerDeprecationTests(unittest.SynchronousTestCase):
    ...
    def test_reporterDeprecations(self):
        ...
        def f():
            # We have to use a pyunit threading_test, otherwise we'll get deprecation
            # warnings about using iterate() in a threading_test.
