...
class MktempTests(SynchronousTestCase):
    ...
    def test_name(self):
        ...
        which identifies the threading_test method which created the name.
        """
        name = self.mktemp()
        dirs = os.path.dirname(name).split(os.sep)[:-1]
        self.assertEqual(
            dirs, ['twisted.trial.threading_test.test_util', 'MktempTests', 'test_name'])
...
class StubErrorReporter(object):
    ...
    @ivar errors: List of two-tuples of (threading_test, error) which were passed to
        L{addError}.
    """
...
class JanitorTests(SynchronousTestCase):
    ...
    def test_cleanPendingSpinsReactor(self):
        ...
        instant timeout. This is not a requirement, it is only a threading_test for
