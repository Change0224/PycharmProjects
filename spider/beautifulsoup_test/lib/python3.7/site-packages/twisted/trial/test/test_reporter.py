...
class ErrorReportingTests(StringTest):
    ...
    def test_formatErroredMethod(self):
        """
        A threading_test method which runs and has an error recorded against it is
        reported in the output stream with the I{ERROR} tag along with a
        summary of what error was reported and the ID of the threading_test.
        """
        ...
        match = [
            ...
            ('twisted.trial.threading_test.erroneous.FoolishError: '
    ...
    def test_formatFailedMethod(self):
        """
        A threading_test method which runs and has a failure recorded against it is
        reported in the output stream with the I{FAIL} tag along with a summary
        of what failure was reported and the ID of the threading_test.
        """
        suite = self.loader.loadByName(
            "twisted.trial.threading_test.erroneous.TestRegularFail.test_fail")
        output = self.getOutput(suite).splitlines()
        match = [
            self.doubleSeparator,
            '[FAIL]',
    ...
    def test_doctestError(self):
        ...
        problem was encountered and the ID of the threading_test.
        """
        from twisted.trial.test import erroneous
        suite = unittest.decorate(
            self.loader.loadDoctests(erroneous), itrial.ITestCase)
        output = self.getOutput(suite)
        path = 'twisted.trial.threading_test.erroneous.unexpectedException'
    ...
    def test_hiddenException(self):
        ...
        threading_test already has a failure.
        ...
        if reflect.qual(reactor).startswith("twisted.internet.asyncioreactor"):
            raise self.skipTest(
                ("This threading_test does not work on the asyncio reactor, as the "
        ...
        match = [
            ...
            'twisted.trial.threading_test.erroneous.DelayedCall.testHiddenException',
            self.doubleSeparator,
            '[ERROR]',
            'Traceback (most recent call last):',
...
class TracebackHandlingTests(unittest.SynchronousTestCase):
    ...
    def getErrorFrames(self, test):
        """
        Run the given C{threading_test}, make sure it fails and return the trimmed
        frames.

        @param test: The threading_test case to run.
    ...
    def test_basic(self):
        ...
        self.checkFrames(frames,
                         [('test_fail', 'twisted/trial/threading_test/erroneous')])

    def test_subroutine(self):
        test = erroneous.TestRegularFail('test_subfail')
        frames = self.getErrorFrames(test)
        self.checkFrames(frames,
                         [('test_subfail', 'twisted/trial/threading_test/erroneous'),
                          ('subroutine', 'twisted/trial/threading_test/erroneous')])

    def test_deferred(self):
        """
        C{_trimFrames} removes traces of C{_runCallbacks} when getting an error
        in a callback returned by a C{TestCase} based threading_test.
        """
        test = erroneous.TestAsynchronousFail('test_fail')
        frames = self.getErrorFrames(test)
        self.checkFrames(frames,
                         [('_later', 'twisted/trial/threading_test/erroneous')])
    ...
    def test_exception(self):
        ...
        based threading_test.
        """
        test = erroneous.TestAsynchronousFail('test_exception')
        frames = self.getErrorFrames(test)
        self.checkFrames(frames,
                         [('test_exception', 'twisted/trial/threading_test/erroneous')])
...
class PyunitNamesTests(unittest.SynchronousTestCase):
    ...
    def test_verboseReporter(self):
        ...
        self.assertEqual(
            output, 'twisted.trial.threading_test.sample.PyunitTest.test_foo ... ')
    ...
    def test_emptyMinimalReporter(self):
        ...
        threading_test is actually run.
        """
        result = reporter.MinimalReporter(self.stream)
        result._printSummary()
        output = self.stream.getvalue().strip().split(' ')
...
class DirtyReactorTests(unittest.SynchronousTestCase):
    ...
    warnings, as a migration tool for threading_test authors. It causes a wrapper to be
...
class SkipTests(unittest.SynchronousTestCase):
    ...
    def test_summary(self):
        """
        The summary of a successful run with skips indicates that the threading_test
    ...
    def test_basicErrors(self):
        """
        The output at the end of a threading_test run with skips includes the reasons
    ...
    def test_booleanSkip(self):
        ...
        attribute to True. When this happens, the threading_test output includes 'True'
        as the reason.
        """
    ...
    def test_exceptionSkip(self):
        ...
        included in the summary at the end of the threading_test suite.
        """
        try:
            1/0
...
class TodoTests(unittest.SynchronousTestCase):
    ...
    def test_success(self):
        """
        A threading_test run is still successful even if there are expected failures.
        """
        self.result.addExpectedFailure(self.test, Failure(Exception()),
                                       makeTodo('todo!'))
        self.assertEqual(True, self.result.wasSuccessful())
    ...
    def test_unexpectedSuccess(self):
        """
        A threading_test which is marked as todo but succeeds will have an unexpected
        success reported to its result. A threading_test run is still successful even
        when this happens.
        """
    ...
    def test_booleanTodo(self):
        ...
        is a threading_test for current behavior, not a requirement.
        """
...
class TreeReporterTests(unittest.SynchronousTestCase):
    ...
    def test_getPrelude(self):
        """
        The tree needs to get the segments of the threading_test ID that correspond
...
class ReporterInterfaceTests(unittest.SynchronousTestCase):
    ...
    Subclass this threading_test case and provide a different 'resultFactory' to threading_test
...
class ReporterTests(ReporterInterfaceTests):
    ...
    def test_duplicateWarningSuppressed(self):
        """
        A warning emitted twice within a single threading_test is only written to the
        stream once.
        """
    ...
    def test_warningEmittedForNewTest(self):
        """
        A warning emitted again after a new threading_test has started is written to the
        stream again.
        """
        ...
        # Stop the first threading_test and start another one (it just happens to be the
...
class SubunitReporterTests(ReporterInterfaceTests):
    ...
    def setUp(self):
        if reporter.TestProtocolClient is None:
            raise SkipTest(
                "Subunit not installed, cannot threading_test SubunitReporter")
    ...
    def test_addExpectedFailurePassedThrough(self):
        ...
        versions, when we call 'addExpectedFailure' on the threading_test result, we
        pass the error and threading_test through to the subunit client.
        """
    ...
    def test_addSkipSendsSubunitAddSkip(self):
        ...
        call 'addSkip' on the threading_test result, we pass the threading_test and reason through
...
class LoggingReporter(reporter.Reporter):
    """
    Simple reporter that stores the last threading_test that was passed to it.
    """
...
class AdaptedReporterTests(unittest.SynchronousTestCase):
    ...
    def test_addError(self):
        """
        C{addError} wraps its threading_test with the provided adapter.
        """
    ...
    def test_addFailure(self):
        """
        C{addFailure} wraps its threading_test with the provided adapter.
        """
    ...
    def test_addSkip(self):
        """
        C{addSkip} wraps its threading_test with the provided adapter.
        """
    ...
    def test_startTest(self):
        """
        C{startTest} wraps its threading_test with the provided adapter.
        """
    ...
    def test_stopTest(self):
        """
        C{stopTest} wraps its threading_test with the provided adapter.
        """
    ...
    def test_addExpectedFailure(self):
        """
        C{addExpectedFailure} wraps its threading_test with the provided adapter.
        """
    ...
    def test_addUnexpectedSuccess(self):
        """
        C{addUnexpectedSuccess} wraps its threading_test with the provided adapter.
        """
