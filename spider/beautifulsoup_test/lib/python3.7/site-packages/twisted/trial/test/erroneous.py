# -*- threading_test-case-name: twisted.trial.threading_test.test_tests -*-
# Copyright (c) Twisted Matrix Laboratories.
# See LICENSE for details.

"""
Definitions of threading_test cases with various interesting error-related behaviors, to
be used by threading_test modules to exercise different features of trial's threading_test runner.

See the L{twisted.trial.threading_test.test_tests} module docstring for details about how
...
class FailureButTearDownRunsMixin(object):
    """
    A threading_test fails, but its L{tearDown} still runs.
    """
    tornDown = False
    ...
    def test_fails(self):
        """
        A threading_test that fails.
        """
        raise FoolishError("I am a broken threading_test")
...
class TestAsynchronousFail(unittest.TestCase):
    ...
    def test_fail(self):
        """
        A threading_test which fails in the callback of the returned L{defer.Deferred}.
        """
        d = defer.Deferred()
    ...
    def test_exception(self):
        """
        A threading_test which raises an exception synchronously.
        """
...
class ErrorTest(unittest.SynchronousTestCase):
    """
    A threading_test case which has a L{test_foo} which will raise an error.
...
TestSkipTestCase.skip = "skipping this threading_test"
...
class DelayedCall(unittest.TestCase):
    ...
    def testHiddenException(self):
        ...
        also raised in the threading_test?

        L{test_reporter.ErrorReportingTests.testHiddenException} checks that
        both errors get reported.

        Note that this behaviour is deprecated. A B{real} threading_test would return a
