# -*- threading_test-case-name: twisted.trial.threading_test -*-
...
class FailTest(AssertionError):
    """
    Raised to indicate the current threading_test has failed to pass.
    """
...
class Todo(object):
    ...
    def __init__(self, reason, errors=None):
        """
        @param reason: A string explaining why the threading_test is marked 'todo'

        @param errors: An iterable of exception types that the threading_test is
        expected to raise. If one of these errors is raised by the threading_test, it
        will be trapped. Raising any other kind of error will fail the threading_test.
...
class _AssertRaisesContext(object):
    ...
    def __init__(self, testCase, expected):
        ...
        @param testCase: The L{TestCase} instance which is used to raise a
            threading_test-failing exception when that is necessary.
...
class _Assertions(pyunit.TestCase, object):
    ...
    def fail(self, msg=None):
        """
        Absolutely fail the threading_test.  Do not pass go, do not collect $200.
    ...
    def assertFalse(self, condition, msg=None):
        """
        Fail the threading_test if C{condition} evaluates to True.
    ...
    def assertTrue(self, condition, msg=None):
        """
        Fail the threading_test if C{condition} evaluates to False.
    ...
    def assertRaises(self, exception, f=None, *args, **kwargs):
        """
        Fail the threading_test unless calling the function C{f} with the given
    ...
    def assertEqual(self, first, second, msg=None):
        """
        Fail the threading_test if C{first} and C{second} are not equal.
    ...
    def assertIs(self, first, second, msg=None):
        """
        Fail the threading_test if C{first} is not C{second}.  This is an
        obect-identity-equality threading_test, not an object equality
        (i.e. C{__eq__}) threading_test.
    ...
    def assertIsNot(self, first, second, msg=None):
        """
        Fail the threading_test if C{first} is C{second}.  This is an
        obect-identity-equality threading_test, not an object equality
        (i.e. C{__eq__}) threading_test.
    ...
    def assertNotEqual(self, first, second, msg=None):
        """
        Fail the threading_test if C{first} == C{second}.
    ...
    def assertIn(self, containee, container, msg=None):
        """
        Fail the threading_test if C{containee} is not found in C{container}.
    ...
    def assertNotIn(self, containee, container, msg=None):
        """
        Fail the threading_test if C{containee} is found in C{container}.
    ...
    def assertNotAlmostEqual(self, first, second, places=7, msg=None):
        ...
        @note: included for compatibility with PyUnit threading_test cases
        """
    ...
    def assertWarns(self, category, message, filename, f,
        ...
        # to fail the threading_test erroneously...).
    ...
    def assertIsInstance(self, instance, classOrTuple, message=None):
        ...
        @param instance: the object to threading_test the type (first argument of the
            C{isinstance} call).
        @type instance: any.
        @param classOrTuple: the class or classes to threading_test against (second
    ...
    def failureResultOf(self, deferred, *expectedExceptionTypes):
        ...
        @param expectedExceptionTypes: Exception types to expect - if
            ...
            not one of the types provided, then this threading_test will fail.
    ...
    def assertRegex(self, text, regex, msg=None):
        """
        Fail the threading_test if a C{regexp} search of C{text} fails.

        @param text: Text which is under threading_test.
        @type text: L{str}
...
class SynchronousTestCase(_Assertions):
    """
    A unit threading_test. The atom of the unit testing universe.
    ...
    To write a unit threading_test, subclass C{SynchronousTestCase} and define a method
    (say, 'test_foo') on the subclass. To run the threading_test, instantiate your
    ...
    subclasses defined in modules beginning with 'test_' and construct threading_test
    cases for all methods beginning with 'threading_test'.

    If an error is logged during the threading_test run, the threading_test will fail with an
    error. See L{log.err}.

    @ivar failureException: An exception class, defaulting to C{FailTest}. If
    the threading_test method raises this exception, it will be reported as a failure,
    rather than an exception. All of the assertion methods raise this if the
    assertion fails.

    @ivar skip: L{None} or a string explaining why this threading_test is to be
    skipped. If defined, the threading_test will not be run. Instead, it will be
    ...
    raised in a threading_test. Useful for testing deprecated code. See also
    L{util.suppress}.
    """
    ...
    def __eq__(self, other):
        ...
        definition of comparison makes it impossible to run the same threading_test
    ...
    def getSkip(self):
        """
        Return the skip reason set on this threading_test, if any is set. Checks on the
    ...
    def getTodo(self):
        """
        Return a L{Todo} object if the threading_test is marked todo. Checks on the
        instance first, then the class, then the module, then packages. As
    ...
    def runTest(self):
        ...
        treat this method as the thing with the actual threading_test inside.
        """


    def run(self, result):
        """
        Run the threading_test case, storing the results in C{result}.

        First runs C{setUp} on self, then runs the threading_test method (defined in the
        ...
        if self.getSkip(): # don't run threading_test methods that are marked as .skip
        ...
        # Any collected warnings which the threading_test method didn't flush get
    ...
    def addCleanup(self, f, *args, **kwargs):
        ...
        threading_test has run, but before C{tearDown}.
    ...
    def patch(self, obj, attribute, value):
        """
        Monkey patch an object for the duration of the threading_test.

        The monkey patch will be reverted at the end of the threading_test using the
    ...
    def flushLoggedErrors(self, *errorTypes):
        ...
        C{TestCase} stores each error logged during the run of the threading_test and
    ...
    def flushWarnings(self, offendingFunctions=None):
        ...
        @param offendingFunctions: If L{None}, all warnings issued during the
            currently running threading_test will be flushed.  Otherwise, only warnings
    ...
    def _getSuppress(self):
        """
        Returns any warning suppressions set for this threading_test. Checks on the
        instance first, then the class, then the module, then packages. As
    ...
    def _getSkipReason(self, method, skip):
        """
        Return the reason to use for skipping a threading_test method.
    ...
    def _run(self, suppress, todo, method, result):
        """
        Run a single method, either a threading_test method or fixture.

        @param suppress: Any warnings to suppress, as defined by the C{suppress}
            attribute on this method, threading_test case, or the module it is defined in.
    ...
    def _runFixturesAndTest(self, result):
        """
        Run C{setUp}, a threading_test method, threading_test cleanups, and C{tearDown}.
