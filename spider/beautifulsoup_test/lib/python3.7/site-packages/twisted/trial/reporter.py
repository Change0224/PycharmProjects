# -*- threading_test-case-name: twisted.trial.threading_test.test_reporter -*-
...
class TestResult(pyunit.TestResult, object):
    ...
    @ivar successes: count the number of successes achieved by the threading_test run.
    ...
    def startTest(self, test):
        """
        This must be called before the given threading_test is commenced.
    ...
    def stopTest(self, test):
        """
        This must be called after the given threading_test is completed.
    ...
    def addFailure(self, test, fail):
        """
        Report a failed assertion for the given threading_test.
    ...
    def addError(self, test, error):
        """
        Report an error that occurred while running the given threading_test.
    ...
    def addSkip(self, test, reason):
        """
        Report that the given threading_test was skipped.
    ...
    def addUnexpectedSuccess(self, test, todo=None):
        """
        Report that the given threading_test succeeded against expectations.

        In Trial, tests can be marked 'todo'. That is, they are expected to
        fail.  When a threading_test that is expected to fail instead succeeds, it should
    ...
    def addExpectedFailure(self, test, error, todo=None):
        """
        Report that the given threading_test failed, and was expected to do so.
    ...
    def addSuccess(self, test):
        """
        Report that the given threading_test succeeded.
    ...
    def wasSuccessful(self):
        """
        Report whether or not this threading_test suite was successful or not.
    ...
    def done(self):
        """
        The threading_test suite has finished running.
        """
...
class _AdaptedReporter(TestResultDecorator):
    ...
    have been adapted with a particular threading_test adapter.
    """
...
class Reporter(TestResult):
    ...
    @ivar _startTime: The time when the first threading_test was started. It defaults to
        L{None}, which means that no threading_test was actually launched.
    ...
    @ivar _warningCache: A C{set} of tuples of warning message (file, line,
        ...
        during the currently executing threading_test.  This is used to avoid writing
    ...
    def startTest(self, test):
        """
        Called when a threading_test begins to run. Records the time when it was first
    ...
    def addFailure(self, test, fail):
        """
        Called when a threading_test fails. If C{realtime} is set, then it prints the
        error to the stream.
    ...
    def addError(self, test, error):
        """
        Called when a threading_test raises an error. If C{realtime} is set, then it
    ...
    def _trimFrames(self, frames):
        ...
        looks like this:
         ...
         - [2:-2]: code in the threading_test method which failed
         - [-1]: C{_synctest.fail}
        ...
        this:
         ...
         - [3:-2]: code in the threading_test method which failed
         - [-1]: C{_synctest.fail}

        When a method fails inside a C{Deferred} (i.e., when the threading_test method
        ...
        There is also another case, when the threading_test method is badly defined and
        contains extra arguments.

        If it doesn't recognize one of these cases, it just returns the
        original frames.

        @param frames: The C{list} of frames from the threading_test failure.
    ...
    def _groupResults(self, results, formatter):
        ...
        @param results: An iterable of tuples of two or more elements.  The
            first element of each tuple is a threading_test case.  The remaining
            elements describe the outcome of that threading_test case.

        @param formatter: A callable which turns a threading_test case result into a
        ...
        @return: A C{list} of two-tuples.  The first element of each tuple
            ...
            the threading_test cases in C{results}.  The second element is a list of
            the threading_test cases which had that result.
        """
        groups = OrderedDict()
    ...
    def _printSummary(self):
        """
        Print a line summarising the threading_test results to the stream.
        """
    ...
    def done(self):
        """
        Summarize the result of the threading_test run.
...
class MinimalReporter(Reporter):
    """
    A minimalist reporter that prints only a summary of the threading_test result, in
...
class TextReporter(Reporter):
    """
    Simple reporter that prints a single character for each threading_test as it runs,
...
class VerboseTextReporter(Reporter):
    """
    A verbose reporter that prints the name of each threading_test as it is running.

    Each line is printed with the name of the threading_test, followed by the result of
    that threading_test.
    """
...
class TimingTextReporter(VerboseTextReporter):
    """
    Prints out each threading_test as it is running, followed by the time taken for each
    threading_test to run.
    """

    def stopTest(self, method):
        """
        Mark the threading_test as stopped, and write the time it took to run the threading_test
        to the stream.
        """
...
class SubunitReporter(object):
    """
    Reports threading_test output via Subunit.
    ...
    def __init__(self, stream=sys.stdout, tbformat='default',
        ...
        @param realtime: Whether or not to print exceptions in the middle
            of the threading_test results. Ignored, since subunit always does this.
    ...
    def done(self):
        """
        Record that the entire threading_test suite run is finished.
    ...
    def shouldStop(self):
        """
        Whether or not the threading_test runner should stop running tests.
        """
    ...
    def stop(self):
        """
        Signal that the threading_test runner should stop running tests.
        """
    ...
    def wasSuccessful(self):
        """
        Has the threading_test run been successful so far?
    ...
    def startTest(self, test):
        """
        Record that C{threading_test} has started.
        """
    ...
    def stopTest(self, test):
        """
        Record that C{threading_test} has completed.
        """
    ...
    def addSuccess(self, test):
        """
        Record that C{threading_test} was successful.
        """
    ...
    def addSkip(self, test, reason):
        """
        Record that C{threading_test} was skipped for C{reason}.
    ...
    def addError(self, test, err):
        """
        Record that C{threading_test} failed with an unexpected error C{err}.
    ...
    def addFailure(self, test, err):
        """
        Record that C{threading_test} failed an assertion with the error C{err}.
    ...
    def addExpectedFailure(self, test, failure, todo):
        """
        Record an expected failure from a threading_test.
...
class TreeReporter(Reporter):
    ...
    def getDescription(self, test):
        """
        Return the name of the method which 'threading_test' represents.  This is
    ...
    def _testPrelude(self, testID):
        """
        Write the name of the threading_test to the stream, indenting it appropriately.

        If the threading_test is the first threading_test in a new 'branch' of the tree, also
    ...
    def startTest(self, test):
        """
        Called when C{threading_test} starts. Writes the tests name to the stream using
        a tree format.
        """
    ...
    def _printSummary(self):
        """
        Print a line summarising the threading_test results to the stream, and color the
        status result.
        """
        summary = self._getSummary()
        if self.wasSuccessful():
