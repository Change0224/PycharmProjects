# -*- threading_test-case-name: twisted.trial.threading_test.test_script -*-
...
def getTestModules(filename):
    testCaseVar = loadLocalVariables(filename).get('threading_test-case-name', None)
...
def _maybeFindSourceLine(testThing):
    """
    Try to find the source line of the given threading_test thing.

    @param testThing: the threading_test item to attempt to inspect
    @type testThing: an L{TestCase}, threading_test method, or module, though only the
        former two have a chance to succeed
    @rtype: int
    @return: the starting source line, or -1 if one couldn't be found
    """

    # an instance of L{TestCase} -- locate the threading_test it will run
...
_runOrders = {
    "alphabetical" : (
        "alphabetical order for threading_test methods, arbitrary order for threading_test cases",
        runner.name),
    "toptobottom" : (
     "attempt to run threading_test cases and methods in the order they were defined",
...
def _checkKnownRunOrder(order):
    """
    Check that the given order is a known threading_test running order.
...
class _BasicOptions(object):
    ...
    optFlags = [["help", "h"],
                ...
                ['help-orders', None, "Help on available threading_test running orders"],
                ['help-reporters', None,
                ...
                ["force-gc", None, "Have Trial run gc.collect() before and "
                 "after each threading_test case."],
                ["exitfirst", "x",
    ...
    optParameters = [
        ["order", "o", None,
         "Specify what order to run threading_test cases and methods. "
        ...
        ['reporter', None, 'verbose',
         'The reporter to use for this threading_test run.  See --help-reporters for '
         'more info.']]
    ...
    def opt_testmodule(self, filename):
        """
        Filename to grep for threading_test cases (-*- threading_test-case-name).
        """
        # If the filename passed to this parameter looks like a threading_test module
        # we just add that to the threading_test suite.
        #
        # If not, we inspect it for an Emacs buffer local variable called
        # 'threading_test-case-name'.  If that variable is declared, we try to add its
        # value to the threading_test suite as a module.
        #
    ...
    def opt_help_orders(self):
        synopsis = ("Trial can attempt to run threading_test cases and their methods in "
...
class Options(_BasicOptions, usage.Options, app.ReactorSelectionMixin):
    ...
    optFlags = [
                ...
                ["until-failure", "u", "Repeat threading_test until it fails"],
                ]

    optParameters = [
        ["debugger", None, "pdb", "the fully qualified name of a debugger to "
         "use if --debug is passed"],
        ["logfile", "l", "threading_test.log", "log file name"],
