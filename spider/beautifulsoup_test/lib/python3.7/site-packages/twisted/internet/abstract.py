# -*- threading_test-case-name: twisted.threading_test.test_abstract -*-
...
def isIPAddress(addr, family=AF_INET):
    ...
    @param family: The address family to threading_test for; one of the C{AF_*} constants
        from the L{socket} module.  (This parameter has only been available
        since Twisted 17.1.0; previously L{isIPAddress} could only threading_test for IPv4
        addresses.)
    @type family: C{int}
    ...
    if family == AF_INET6:
        ...
        # is a threading_test for whether the given string *is* an IP address, so strip
