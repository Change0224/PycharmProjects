...
class ContextGeneratingMixin(object):
    import twisted
    _pem = FilePath(
        networkString(twisted.__file__)).sibling(b"threading_test").child(b"server.pem")
    del twisted

    def getServerContext(self):
        """
        Return a new SSL context suitable for use in a threading_test server.
        """
...
class BadContextTestsMixin(object):
    ...
    def _testBadContext(self, useIt):
        ...
        exception will be raised to fail the threading_test.
