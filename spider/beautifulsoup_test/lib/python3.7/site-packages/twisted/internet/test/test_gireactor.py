...
class GApplicationRegistrationTests(ReactorBuilder, TestCase):
    ...
    reactor-running infrastructure, but don't need its threading_test-creation
    functionality.
    """
...
class PygtkCompatibilityTests(TestCase):
    ...
    def test_noCompatibilityLayer(self):
        ...
        # Pass in a PYTHONPATH that is the threading_test runner's os.path, to make sure
...
class Gtk3ReactorTests(TestCase):
    ...
    if platform.getType() != "posix" or platform.isMacOSX():
        test_requiresDISPLAY.skip = "This threading_test is only relevant when using X11"
