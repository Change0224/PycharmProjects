...
def needsRunningReactor(reactor, thunk):
    ...
    some point.  They need to stop the reactor when the threading_test has completed, and
    ...
    a particular API under threading_test returns fires synchronously (as especially an
    endpoint's C{connect()} method may do, if the connect is to a local
    interface address) then the threading_test won't be able to stop the reactor being
    ...
    @param reactor: the L{twisted.internet.interfaces.IReactorCore} under threading_test

    @param thunk: a 0-argument callable, which eventually finishes the threading_test in
...
def stopOnError(case, reactor, publisher=None):
    ...
    threading_test would have to complete by timing out which is a much less desirable
    ...
    @param case: A L{SynchronousTestCase} to use to clean up the necessary log
        observer when the threading_test is over.
...
class ReactorBuilder:
    ...
    if platform.isWindows():
        ...
        # temporarily move this up to the all-platforms list to threading_test
    ...
    def tearDown(self):
        ...
        if process is not None:
            ...
            while process.reapProcessHandlers:
                ...
                # hanging the threading_test suite, eventually give the process some
    ...
    def unbuildReactor(self, reactor):
        ...
        reactor by its creation or by a threading_test which used it.
        """
        # Chris says:
        #
    ...
    def buildReactor(self):
        ...
        try:
            ...
            # and skip the threading_test if necessary.  We also log it to aid
            # with debugging, but flush the logged error so the threading_test
            # doesn't fail.
    ...
    def getTimeout(self):
        """
        Determine how long to run the threading_test before considering it failed.
    ...
    def runReactor(self, reactor, timeout=None):
        ...
        @param timeout: The maximum amount of time, specified in seconds, to
            ...
            raised.  If L{None}, the default threading_test method timeout imposed by
