...
class TCPConnectionTests(TestCase):
    ...
    def test_doReadWarningIsRaised(self):
        ...
        self.assertEqual(
            ...
            "twisted.internet.threading_test.test_tcp.FakeProtocol.dataReceived "
...
class TCPClientTestsBase(ReactorBuilder, ConnectionTestsMixin,
    ...
    the documented instance variables in order to specify how the threading_test works.
    ...
    @type endpoints: L{twisted.internet.threading_test.connectionmixins.EndpointCreator}
    ...
    @ivar family: an address family constant, such as L{socket.AF_INET},
        ...
        family of the transport type under threading_test.
    @type family: C{int}

    @ivar addressClass: the L{twisted.internet.interfaces.IAddress} implementor
        associated with the transport type under threading_test.  Must also be a
    ...
    def test_buildProtocolReturnsNone(self):
        ...
        # Make sure the threading_test ends quickly.
        ...
        def connected(ignored):
            ...
            # threading_test is primarily concerned with.
        ...
        def disconnected(ignored):
            ...
            # returns None).  If an error got logged it will fail the threading_test.
            # Stop the reactor now so the threading_test can complete one way or the
            # other now.
            reactor.stop()
...
class TCP6ClientTestsBuilder(TCPClientTestsBase):
    ...
    def setUp(self):
        ...
        # This is used by test_addresses to threading_test the distinction between the
        ...
        # it so that the connect call for the IPv6 address threading_test simply uses an
        # address literal.
...
def createTestSocket(test, addressFamily, socketType):
    """
    Create a socket for the duration of the given threading_test.

    @param test: the threading_test to add cleanup to.
...
def assertPeerClosedOnEMFILE(
    ...
    @param testCase: The threading_test case under which to run this assertion.
    ...
    @param reactor: The reactor under threading_test.
...
class AssertPeerClosedOnEMFILETests(SynchronousTestCase):
    ...
    def test_reactorStoppedOnSuccessfulConnection(self):
        ...
        reaches the server, the reactor is stopped and the threading_test fails.
        """
    ...
    def getListeningPort(self, reactor, factory, port=0, interface=''):
        ...
        if IReactorSocket.providedBy(reactor):
            ...
            try:
                ...
                # not.
                portSock.fileno()
                # Now clean it up, because the rest of the threading_test does not need
                # it.
                portSock.close()
        else:
...
def oneTransportTest(testMethod):
    """
    Decorate a L{ReactorBuilder} threading_test function which tests one reactor and one
    connected transport.  Run that threading_test method in the context of
    C{connectionMade}, and immediately drop the connection (and end the threading_test)
    when that completes.

    @param testMethod: A unit threading_test method on a L{ReactorBuilder} threading_test suite;
    ...
    @return: a no-argument threading_test method.
...
def assertReading(testCase, reactor, transport):
    """
    Use the given threading_test to assert that the given transport is actively reading
    in the given reactor.

    @note: Maintainers; for more information on why this is a function rather
        than a method on a threading_test case, see U{this document on how we structure
        threading_test tools
        <http://twistedmatrix.com/trac/wiki/Design/KeepTestToolsOutOfFixtures>}

    @param testCase: a threading_test case to perform the assertion upon.
...
def assertNotReading(testCase, reactor, transport):
    """
    Use the given threading_test to assert that the given transport is I{not} actively
...
class TCPConnectionTestsBuilder(ReactorBuilder):
    ...
    def test_stopStartReading(self):
        """
        This threading_test verifies transport socket read state after multiple
        ...
        if reactorClassName in skippedReactors and platform.isWindows():
            raise SkipTest(
                "This threading_test is broken on gtk/glib under Windows.")
    ...
    def test_doubleHalfClose(self):
        ...
        # If threading_test fails, reactor won't stop and we'll hit timeout:
...
class TCPTransportServerAddressTestMixin(object):
    ...
    def _testServerAddress(self, interface, addressFamily, adressClass):
        """
        Helper method to threading_test TCP server addresses on either IPv4 or IPv6.
        """
        ...
        def connected(protocols):
            ...
            try:
                ...
                # Be certain to drop the connection so the threading_test completes.
                server.transport.loseConnection()
...
class AbortConnectionMixin(object):
    ...
    def runAbortTest(self, clientClass, serverClass,
        ...
        """
        A threading_test runner utility function, which hooks up a matched pair of client
...
class BuffersLogsTests(SynchronousTestCase):
    ...
    def test_flushesOnExitWithException(self):
        ...
        class TestException(Exception):
            """
            An exception only raised by this threading_test.
            """
...
class FileDescriptorReservationTests(SynchronousTestCase):
    ...
    def test_exitOpensFileOnException(self):
        ...
        class TestException(Exception):
            """
            An exception only used by this threading_test.
            """
