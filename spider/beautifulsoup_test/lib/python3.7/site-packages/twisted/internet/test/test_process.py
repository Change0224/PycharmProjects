...
def onlyOnPOSIX(testMethod):
    """
    Only run this threading_test on POSIX platforms.

    @param testMethod: A threading_test function, being decorated.
...
class ProcessTestsBuilderBase(ReactorBuilder):
    ...
    def test_writeToChildBadFileDescriptor(self):
        ...
        def write(transport, bytesToSend):
            try:
                ...
                # Just get the process to exit so the threading_test can complete
                transport.write(bytesToSend)
        self._writeTest(write)
    ...
    if getattr(signal, 'SIGCHLD', None) is None:
        test_spawnProcessEarlyIsReaped.skip = (
            "Platform lacks SIGCHLD, early-spawnProcess threading_test can't work.")
    ...
    def test_processExitedWithSignal(self):
        ...
        class Exiter(ProcessProtocol):
            ...
            def processExited(self, reason):
                ...
                # Exception would also make an errback-based threading_test pass, and
                # that would be wrong.
    ...
    def test_openFileDescriptors(self):
        ...
        # To threading_test this, we are going to open a file descriptor in the parent
...
sys.stdout.flush()""".format(twistedRoot.path))
        ...
        # might open some and this threading_test should pass in those environments.
    ...
    def _changeIDTest(self, which):
        ...
        raise an exception to fail the threading_test.
    ...
    def test_processExitedRaises(self):
        ...
        # Only required if the threading_test fails on systems that support
        # the process module.
...
class ProcessTestsBuilder(ProcessTestsBuilderBase):
    ...
    keepStdioOpenProgram = b'twisted.internet.threading_test.process_helper'
    if platform.isWindows():
    ...
    # Define this threading_test here because PTY-using processes only have stdin and
    # stdout and the threading_test would need to be different for that to work.
    def test_childConnectionLost(self):
        ...
        target = b"twisted.internet.threading_test.process_loseconnection"
    ...
    # This threading_test is here because PTYProcess never delivers childConnectionLost.
    ...
    # This threading_test is here because PTYProcess.loseConnection does not actually
    # close the file descriptors to the child process.  This threading_test needs to be
    ...
    def test_processCommandLineArguments(self):
        ...
        originally intended.
        """
        us = b"twisted.internet.threading_test.process_cli"
