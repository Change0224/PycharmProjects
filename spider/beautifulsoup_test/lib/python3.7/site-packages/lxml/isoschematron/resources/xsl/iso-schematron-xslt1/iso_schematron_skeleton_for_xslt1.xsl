...
<!--
 ...
 Conventions: Meta-stylesheets that override this may use the following parameters
    ...
    diagnose= yes | no    Add the diagnostics to the assertion threading_test in reports
...
<!--
  ...
  VERSION INFORMATION 
   ...
   Version: 2008-07-14 minor update for inclusion experiments
   	* RJ Clean up zero-length fragment threading_test on include
   	* RJ Add experimental support for include containers 
   	* RJ For path generation, threading_test for //iso:schema not just /iso:schema, for potential embedded Schematron support
   ...
   Version: 2007-04-04
   	* RJ debug mode param
	* RJ alter mixed threading_test to only threading_test mixed branches, so the same document
...
<!--
			<axsl:choose>
				<axsl:when threading_test="count(. | ../namespace::*) = count(../namespace::*)">
   ...
   <xsl:template match="iso:include" mode="do-all-patterns">
	...
	<xsl:template match="iso:pattern[not(@abstract='true')]">
      ...
      <xsl:if test="($phase = '#ALL') 
		...
		<!-- DPC select-contexts threading_test -->
...
<xsl:template name="handle-namespace">
       ...
       <xsl:choose>
        ...
        <!-- Not tested yet       
    	<xsl:when threading_test="function-available('msxsl:node-set')">
      		<xsl:variable name="ns-dummy-elements">
...
<!-- ============================================================== -->
	...
	<!-- Swallow old namespace elements: there is an upfront threading_test for them elsewhere -->
