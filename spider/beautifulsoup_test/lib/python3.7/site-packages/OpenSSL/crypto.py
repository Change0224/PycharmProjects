...
def _untested_error(where):
    ...
    encountered isn't one that's exercised by the threading_test suite so future behavior
...
def _get_asn1_time(timestamp):
    ...
    if _lib.ASN1_STRING_length(string_timestamp) == 0:
        ...
        if generalized_timestamp[0] == _ffi.NULL:
            ...
            #
            # These are difficult to threading_test.  cffi enforces the ASN1_TIME type.
...
# using encryption.
#
# Thus OpenSSL.threading_test.test_crypto.FunctionTests.test_dump_privatekey_passphrase
