        ...
        while 1:
            ...
            for num in range(1, len(tr) + 1):
                ...
                if existed_topic:
                    ...
                    if topic.topic_url:
                        ...
                        topic_detail_urls.append(topic_url)
            ...
            if sel.css(next_page_css).extract():
                ...
                if '?' in url and length == 1:
                    ...
                    # print(next_url)
                    topic_detail_urls.append(next_url)
...
class PraseTopicDetailTread(Thread):
    def run(self):
        while 1:
            try:
                url = topic_detail_urls.pop()
                #url = 'https://bbs.csdn.net/topics/320192800'
            except IndexError:
                time.sleep(1)
                continue
            # 补充topic表中的内容
...
def process_nodes_list(nodes_list):
    ...
    for item in nodes_list:
        if 'url' in item:
            if item['url']:
                topic_list_urls.append(item['url'])
            if 'children' in item:
...
def get_last_list(level1_url):
    last_url = []
    for url in topic_list_urls:
        if url not in level1_url:
            last_url.append(parse.urljoin(domin, url))
            last_url.append(parse.urljoin(domin, url + "/recommend"))
            last_url.append(parse.urljoin(domin, url + "/closed"))
...
if __name__ == '__main__':
    ...
    # 去除一级链接后的链接
    topic_list_urls = get_last_list(level1_url)
