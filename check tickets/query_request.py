...
def query(date,from_station,to_station):
    ...
    'Cookie':'JSESSIONID=883D71E4F6A0BEEB73E972109304BBC6; RAIL_EXPIRATION=1581099242916; RAIL_DEVICEID=RfzsDqFosraMqksfwpi61X37hCwdEd43FZiwVEbXVj7mTDRxrBZXPU5JBVRIZUu1zPHGNzQ6LiUlIGl5PdmSpsdt0bnHd0XNN_yrB9xgzNlmOgX3oGtszyMIEzSzFNW79p3-SG0ot64PhmQhAa_80o_Pkc55LaAS; _jc_save_fromStation=%u5317%u4EAC%2CBJP; _jc_save_toStation=%u65B0%u4E61%2CXXF; _jc_save_fromDate=2020-02-14; _jc_save_toDate=2020-02-05; _jc_save_wfdc_flag=dc; BIGipServerpassport=803733770.50215.0000; route=c5c62a339e7744272a54643b3be5bf64; BIGipServerotn=602407178.50210.0000'
    }
    #发送查询请求
    ...
    if isStations() == True:
        ...
        if len(result) != 0:
            for i in result:
                ...
                #因为出发地和目的地都是缩写字母，所以要在车站库中找到对应对车站名称

                # from_station_EN = tmp_list[6]  #确定起点站对应对英文简称
                # index = list(stations.values()).index(from_station_EN) #从列表中找出某个值第一个匹配项的索引位置
                # from_station = list(stations.keys())[index]
                # print("*"*8)
                # print(from_station)
                # print("*"*8)
